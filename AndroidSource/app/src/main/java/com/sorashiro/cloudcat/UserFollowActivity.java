...
import com.sorashiro.cloudcat.recyclerview.user.UserFollowRVAdapter;
...
public class UserFollowActivity extends AppCompatActivity implements View.OnClickListener {

    private String mUserName;

    private UserFollowRVAdapter mAdapter;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        ArrayList<FollowBean> list = new ArrayList<>();
        mAdapter = new UserFollowRVAdapter(this, list);
    ...
    private void getDataFromServer() {
        ...
        Call<ResponseBody> call = service.getFollow(
                mUserName,
                1
        );
                ...
                if (response.isSuccessful()) {
                    try {
                        ...
                        if (body.contains("resMsg")) {
                            ...
                            LogAndToastUtil.ToastOut(UserFollowActivity.this, show);
                            return;
                        }
                        FollowBean[] beans = gson.fromJson(body, FollowBean[].class);
                        ArrayList<FollowBean> list = new ArrayList<>();
                        list.addAll(Arrays.asList(beans));
                        mAdapter = (UserFollowRVAdapter) listUserFollow.getAdapter();
                        mAdapter.setList(list);
                    } catch (IOException e) {
                        LogAndToastUtil.ToastOut(UserFollowActivity.this, e.getMessage());
                    }
                } else {
                    String error = response.code() + ": " + response.message();
                    LogAndToastUtil.LogV(error);
                    LogAndToastUtil.ToastOut(UserFollowActivity.this,
                            "啊哦，服务器开小差了，请稍候再试");
                }
            }
