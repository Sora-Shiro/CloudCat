...
import com.sorashiro.cloudcat.recyclerview.user.UserFollowingRVAdapter;
...
public class UserFollowingActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private UserFollowingRVAdapter mAdapter;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        mAdapter = new UserFollowingRVAdapter(this, list);
                ...
                if (response.isSuccessful()) {
                    try {
                        ...
                        if (body.contains("resMsg")) {
                            ...
                            LogAndToastUtil.ToastOut(UserFollowingActivity.this, show);
                            return;
                        }
                        FollowBean[] beans = gson.fromJson(body, FollowBean[].class);
                        ArrayList<FollowBean> list = new ArrayList<>();
                        list.addAll(Arrays.asList(beans));
                        mAdapter = (UserFollowingRVAdapter) listUserFollow.getAdapter();
                        mAdapter.setList(list);
                    } catch (IOException e) {
                        LogAndToastUtil.ToastOut(UserFollowingActivity.this, e.getMessage());
                    }
                } else {
                    String error = response.code() + ": " + response.message();
                    LogAndToastUtil.LogV(error);
                    LogAndToastUtil.ToastOut(UserFollowingActivity.this,
                            "啊哦，服务器开小差了，请稍候再试");
                }
            }
