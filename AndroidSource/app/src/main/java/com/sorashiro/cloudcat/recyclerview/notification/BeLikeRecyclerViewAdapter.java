...
public class BeLikeRecyclerViewAdapter extends RecyclerView.Adapter<BeLikeViewHolder> {

    private Context                                       mContext;
    private List<BeLikeBean>                              mList;
    private RecyclerViewClickListener.OnItemClickListener mOnItemClickListener;

    public BeLikeRecyclerViewAdapter(Context context, List<BeLikeBean> lists) {
    ...
    @Override
    public BeLikeViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View itemView = LayoutInflater.from(mContext).inflate(R.layout.item_be_like, parent, false);//填充这个item布局
        BeLikeViewHolder viewHolder = new BeLikeViewHolder(itemView);
        return viewHolder;
    }

    @Override
    public void onBindViewHolder(final BeLikeViewHolder holder, int position) {
        final PosterBean[] innerBean = {null};
        final BeLikeBean beLikeBean = mList.get(position);
        holder.textName.setText(beLikeBean.getName());
        holder.textTime.setText(beLikeBean.getTime());
        holder.textAuthorName.setText(beLikeBean.getAuthor());
        holder.textAuthorPoster.setText(beLikeBean.getOri_text());
        holder.layoutAuthorContent.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (innerBean[0] != null) {
                    ...
                    getInBeanThanStartActivity(innerBean, beLikeBean.getPoster());
                }
            }
        });
        holder.layoutHome.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (innerBean[0] != null) {
                    Intent intent = new Intent(mContext, PosterDetailActivity.class);
                    intent.putExtra("poster_bean", innerBean[0]);
                    intent.putExtra("pdi_type", PDRecyclerViewAdapter.TYPE_LIKE);
                    mContext.startActivity(intent);
                } else {
                    getInBeanThanStartActivity(innerBean, beLikeBean.getPoster());
                }
        ...
        holder.imgHead.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                startUserDetailActivity(beLikeBean.getName());
            }
        });
        holder.imgAuthorHead.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                startUserDetailActivity(beLikeBean.getAuthor());
            }
        });
        Picasso.with(mContext)
                .load(Constant.BASE_STATIC_AVATAR_URL + beLikeBean.getName())
                .placeholder(Constant.AVATAR_DEFAULT)
                .error(Constant.AVATAR_DEFAULT)
                .into(holder.imgHead);
        Picasso.with(mContext)
                .load(Constant.BASE_STATIC_AVATAR_URL + beLikeBean.getAuthor())
                .placeholder(Constant.AVATAR_DEFAULT)
                .error(Constant.AVATAR_DEFAULT)
    ...
    public void setList(List<BeLikeBean> list) {
        mList = list;
