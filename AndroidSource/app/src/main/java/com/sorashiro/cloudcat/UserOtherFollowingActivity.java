...
import com.sorashiro.cloudcat.recyclerview.user.UserOtherFollowingRVAdapter;
...
public class UserOtherFollowingActivity extends AppCompatActivity implements View.OnClickListener {

    private String mUserName;

    private UserOtherFollowingRVAdapter mAdapter;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        mAdapter = new UserOtherFollowingRVAdapter(this, list);
                ...
                if (response.isSuccessful()) {
                    try {
                        ...
                        if (body.contains("resMsg")) {
                            ...
                            LogAndToastUtil.ToastOut(UserOtherFollowingActivity.this, show);
                            return;
                        }
                        OtherFollowBean[] beans = gson.fromJson(body, OtherFollowBean[].class);
                        ArrayList<OtherFollowBean> list = new ArrayList<>();
                        list.addAll(Arrays.asList(beans));
                        mAdapter = (UserOtherFollowingRVAdapter) listUserFollow.getAdapter();
                        mAdapter.setList(list);
                    } catch (IOException e) {
                        LogAndToastUtil.ToastOut(UserOtherFollowingActivity.this, e.getMessage());
                    }
                } else {
                    String error = response.code() + ": " + response.message();
                    LogAndToastUtil.LogV(error);
                    LogAndToastUtil.ToastOut(UserOtherFollowingActivity.this,
                            "啊哦，服务器开小差了，请稍候再试");
                }
            }
